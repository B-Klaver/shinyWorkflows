...
What you'll notice is that a module is very similar to an app, that is, they are composed of two parts:    
...
The second part of this is the server function, which is also going to have an `id` argument, which we will use to link it to our UI component later. Inside of this function we call `moduleServer()` with the `id` arugment, and an additional function that looks like the typical server function:
...
If you'd like to refer to inputs from other places in the app in your module you'll have to pass them to a module argument explicitly - we'll talk about this later in section \@ref(advanced-modules).
...
Let's look at an example from the [book](https://mastering-shiny.org/scaling-modules.html#getting-started-ui-input-server-output) where we will have an additional argument in our input function. We do it the same way as any other function, we add it as an argument in our function call:
...
The next part of the app we will create the server function. Importantly, when we are planning to use an output from a server module we will need to wrap it in a `shiny::reactive()` as it's technically a reactive value given it's relationship with the UI. The example sets up the server like so:
...
Here we put the two parts together. The UI portion is fairly simple, we included the `filter` argument in the larger app call. However, the server part is a bit more complicated. When you are returning a value in the server portion of your app from your module you must assign it to a value in the environment, just like you would in typical R code (ie. `x <- mymodule_server()`. Because we are working with shiny and the returned values from modules are reactive the challenge is deciding if you want to refer to the reactive aspect of the value or the value itself.. let me explain:

- When you want to use that stored value (ie. the dataset or value itself) you must refer to it as `value()` or `data()`
