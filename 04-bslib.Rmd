...
The `bslib` R package provides a modern UI toolkit for Shiny and R Markdown based on Bootstrap, therefore we will not be going over any server-related code as that is all done using `shiny`. It simply builds on `shiny` to help create more intricate and beautiful UI for your app. In summary, `bslib` facilitates:
...
One of the core features of `bslib` is the use of pages or tabs. We all know what this means so I won't go too deep, but in terms of usage it means you will typically be creating `bslib` apps using the `bslib::page_navbar()` function to allow you to create child tabs with `bslib::nav_panel()`. Additionally, sidebars are critical for most apps and many functions, including `bslib::page_navbar()` have a built in `sidebar` argument that you can fill using `bslib::sidebar()`.
...
The last core concept to `bslib` is the way it is geared towards a filling layout, which means that contents are encouraged to grow/shrink to fit the browser window so that regardless of the resolution you're accessing the app it will adjust to fill it nicely. As a result, many functions have a `fill` argument; these are considered "fillable" containers. The child contents - child in this case means it is wrapped by the parent function - will then do the filling feature when `fill = TRUE`, which is the default. When content goes beyond the screen resolution `bslib` will make the content scrollable.
...
DOES THIS NEED TO BE REFERENCED IN THE CARDS???
...
By default, a `card()`’s size grows to accommodate the size of its contents. Thus, if a `card_body()` contains a large amount of text, tables, etc., you may want to specify a `height` or `max_height`. If the contents go beyond the `max_height` the card will become scrollable only if the card's parent is set to `fill = TRUE`, if it isn't then the page becomes scrollable.
...
Although scrolling is convenient for reducing the amount of space required to park lots of content, it can also be a nuisance to the user. When the contents of a card go beyond the height of the screen `bslib` will automatically create a scroll bar for the card. To help reduce the need for scrolling, consider pairing scrolling with `full_screen = TRUE` (which adds an icon to expand the card’s size to the browser window). Alternatively, set the parent to `fill = FALSE` and the whole page will become scrollable.

::::{.tipbox}
:::{.center}
**TIP**
:::
Scrolling in cards can get a bit weird depending on the content. I'd recommend trying always having `full_screen = TRUE` and comparing how scrolling the card feels compared to scrolling the page. Remember, this can differ between pages/tabs!
::::
...
Fill item(s) aren’t limited in how much they grow and shrink, which can be problematic when a card becomes very small. To work around this, consider adding a `min_height` or `height` on the `card_body()` container or alternatively turn `fill` to `FALSE` in the cards parent container (ie. `layout_column_wrap()`).
...
**TIP**
:::
This is one of the best options for setting up a sidebar as it is built in with the `page_navbar()` which you will likely be building your app with anyways!
::::
...
**TIP**
:::
Conditional arguments in the sidebar are useful if certain filters are only applicable to certain pages.
::::
...
**TIP**
:::
Having the sidebar hidden may be useful if the first page is an intro for example.
::::
...
There is a lot of functionality with tooltips and popovers, above are some of the main usages but you can read more and see more examples [here](https://rstudio.github.io/bslib/articles/tooltips-popovers/index.html).
