...
Both `bslib::page_sidebar()` and `bslib::page_navbar()` default to a filling layout, meaning that outputs are encouraged to grow/shrink to fit the browser window. Sometimes this results in undesireable behaviour, you can combat this by setting `bslib::card()` heights using `min_height` to restrict how small a card gets or `max_height` to restrict how large a card will get:
...
To learn everything about filling layouts you can read about it [here](https://rstudio.github.io/bslib/articles/filling/index.html).
...
If you find yourself using `bslib::card_body()` without changing any of its defaults, consider dropping it altogether since it will be wrapped together into an implicit `card_body()` call anyways.
...
If you want to use a single sidebar for multiple pages of an app you want to use the `sidebar` argument that is found in `bslib::page_navbar()` or `bslib::navset_card_tab()`. This is one of the best options for setting up a sidebar.
...
Tooltips and popovers are useful for displaying information in a convenient and non-obtrusive way. Both require a trigger and a message. Tooltips are toggled via focus / hover whereas popovers are toggled via click. As a result, popovers are much more “persistent” (i.e., harder to open/close), and thus should only be used over tooltips when further interaction may be needed. To put it another way, use tooltips for small “read-only” messages, and popovers when the user should be able to interact with the message itself.
...
There is a lot of functionality with tooltips and popovers, above are some of the main usages but you can read more [here](https://rstudio.github.io/bslib/articles/tooltips-popovers/index.html).
